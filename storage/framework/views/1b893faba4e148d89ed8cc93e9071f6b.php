  ...
  <div class="profile-grid mb-28">
    <div class="profile-section" style="grid-column: 1 / -1;">
      ...
      <div class="profile-grid cols-2">
        <div class="form-group">
          ...
          <input type="text" name="hospital_name" class="form-input" value="<?php echo e(old('hospital_name', $Hospital->hospital_name)); ?>" required>
        </div>
        <div class="form-group">
          ...
          <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($Hospital->logo): ?>
            <div style="margin-bottom:8px;"><img src="<?php echo e(asset($Hospital->logo)); ?>" alt="Logo" class="logo-preview" id="logo-preview"></div>
          <?php else: ?>
        ...
        <div class="form-group">
          ...
          <input type="email" name="email" class="form-input" value="<?php echo e(old('email', $Hospital->email)); ?>" placeholder="contact@hospital.org">
        </div>
        <div class="form-group">
          <label class="form-label">Website</label>
          <input type="text" name="website" class="form-input" value="<?php echo e(old('website', $Hospital->website)); ?>" placeholder="https://example.com">
        </div>
        <div class="form-group">
          <label class="form-label">Established year</label>
          <input type="number" name="established_year" class="form-input" value="<?php echo e(old('established_year', $Hospital->established_year)); ?>" min="1800" max="<?php echo e(date('Y')); ?>">
        </div>
        <div class="form-group">
          <label class="form-label">Operating hours</label>
          <input type="text" name="operating_hours" class="form-input" value="<?php echo e(old('operating_hours', $Hospital->operating_hours)); ?>" placeholder="e.g. 24/7 or 8:00 AM – 10:00 PM">
        </div>
        <div class="form-group" style="grid-column: 1 / -1;">
          <label class="form-label">Address</label>
          <input type="text" name="address" class="form-input" value="<?php echo e(old('address', $Hospital->address)); ?>">
        </div>
        <div class="form-group">
          <label class="form-label">City</label>
          <input type="text" name="city" class="form-input" value="<?php echo e(old('city', $Hospital->city)); ?>">
        </div>
        <div class="form-group">
          <label class="form-label">Phone</label>
          <input type="text" name="phone" class="form-input" value="<?php echo e(old('phone', $Hospital->phone)); ?>">
        </div>
        <div class="form-group" style="grid-column: 1 / -1;">
          <label class="form-label">Description</label>
          <textarea name="description" class="form-input" rows="3"><?php echo e(old('description', $Hospital->description)); ?></textarea>
        </div>
      </div>
    </div>
    ...
    <div class="profile-section">
      ...
      <div class="form-group">
        ...
        <input type="number" name="total_rooms" class="form-input" min="0" value="<?php echo e(old('total_rooms', $Hospital->total_rooms ?? 0)); ?>">
      </div>
      <div class="form-group">
        <label class="form-label">Total beds</label>
        <input type="number" name="total_beds" class="form-input" min="0" value="<?php echo e(old('total_beds', $Hospital->total_beds ?? 0)); ?>">
      </div>
      <div class="form-group">
        <label class="form-label">ICU beds</label>
        <input type="number" name="icu_beds" class="form-input" min="0" value="<?php echo e(old('icu_beds', $Hospital->icu_beds ?? 0)); ?>">
      </div>
      <div class="form-group" style="display:flex;gap:20px;flex-wrap:wrap;">
        <label style="display:flex;align-items:center;gap:8px;font-size:.84rem;cursor:pointer;">
          <input type="checkbox" name="emergency_available" value="1" <?php echo e(old('emergency_available', $Hospital->emergency_available) ? 'checked' : ''); ?>>
          24/7 emergency
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:.84rem;cursor:pointer;">
          <input type="checkbox" name="ambulance_available" value="1" <?php echo e(old('ambulance_available', $Hospital->ambulance_available) ? 'checked' : ''); ?>>
          Ambulance service
        </label>
      </div>
    </div>
    ...
    <div class="profile-section" style="grid-column: 1 / -1;">
      ...
      <p style="font-size:.83rem;color:var(--text-sub);margin-bottom:12px;">
        Average rating: <strong id="rating-preview"><?php echo e(number_format((float)($Hospital->rating ?? 0), 1)); ?></strong> / 5 ·
        <span id="reviews-count-preview"><?php echo e((int)($Hospital->total_reviews ?? 0)); ?></span> reviews
      </p>
        ...
        <div class="profile-grid cols-2">
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="new-review-rating">
              <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = range(5,1,-1); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $R): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<script>
(function () {
  let specialties = <?php echo json_encode($Specialties, 15, 512) ?>;
  let facilities = <?php echo json_encode($Facilities, 15, 512) ?>;
  ...
  <?php echo json_encode($Medicines, 15, 512) ?>.forEach(function (m) {
  ...
  <?php echo json_encode($Doctors, 15, 512) ?>.forEach(function (d) { doctorsTbody.appendChild(doctorRow({ name: d.name || '', specialty: d.specialty || '', qualification: d.qualification || '', phone: d.phone || '' })); });
  <?php echo json_encode($SpecialDoctors, 15, 512) ?>.forEach(function (d) { specialDoctorsTbody.appendChild(doctorRow({ name: d.name || '', specialty: d.specialty || '', qualification: d.qualification || '', phone: d.phone || '' })); });
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Aptech\Eprojects\Code\Covid Vacination\resources\views/hospital/profile.blade.php ENDPATH**/ ?>